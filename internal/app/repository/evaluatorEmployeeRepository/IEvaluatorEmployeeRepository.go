...
type IEvaluatorEmployeeRepository interface {
	FindEmployeeIdByEvaluationId(evaluationId, paramEmployeeId int64) (employeeId []int64, err error)
	...
	RetrieveEvaluatorDetailWithPaging(paging datapaging.Datapaging, employeeId int64, departement, search string) (data []evaluatorEmployeesModel.EvaluatorEmployeeList, count int64, err error)
